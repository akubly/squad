...
📌 **Team update (2026-06-02 — Piece 30 Revision Follow-On Review):** FIDO participated in 3-reviewer follow-on adversarial review of Booster's revision (commit a9da5453). Verdict: APPROVE. 0 new mandatory findings. 2 new non-blocking findings: (NB1) remote-add guard mutation not detectable at runtime; (NB2) fixture TEAM_ROOT path mismatch. Mirror byte-identity PASS; test count 196 verified. Ready to merge.

📌 **Team update (2026-06-02 — Piece 30 Revision Follow-on Review, commit a9da5453):** FIDO conducted follow-on quality review of Booster's revision. FIDO M1 RESOLVED — idempotency test uses real pwsh execution (spawnSync -File), asserts run2 exit code 0, no duplicate exclude entries (count == 1 each), no duplicate remotes (≤ 2 in `git remote -v`). PAO M1 RESOLVED — `state-backends` confirmed in EXPECTED_FEATURES. Mirror byte-identity PASS (all 3 files × 4 locations). Test count verified: 196 (14 ado-templates + 181 template-sync + 1 idempotency). Overall verdict: APPROVE. 2 non-blocking nits: (NB1) remote-add guard mutation not detectable by test because git native failure is silent under $ErrorActionPreference = 'Stop' in pwsh — suggest $LASTEXITCODE check; (NB2) fixture TEAM_ROOT path mismatch (team-root vs squad-docs-team) means clone skip guard not exercised. Neither blocks merge.

📌 **Team update (2026-06-03 — Piece 30 Revision, commit a9da5453):** Booster (revision implementer) addressed FIDO's mandatory finding: new test/cli/ado-bootstrap-idempotency.test.ts with execution-based idempotency assertions (6 assertions, pwsh-guarded, 120s timeout). Runs `pwsh -File bootstrap-cross-repo.ps1` twice on fixture; verifies second run is idempotent (no duplicate remotes, no exclude-entry duplication). 196 tests pass (+6 new); quality gate satisfied. Ready for merge.
...
- **`not.toContain` on raw file text is vulnerable to comment false-positives.** When banning a pattern from functional code, filter out comment lines before asserting. `not.toContain('BANNED')` will trip on `# old code used BANNED`. Use `.split('\n').filter(l => !l.trimStart().startsWith('#')).join('\n')` before the assertion.
- **Positive `toContain` guards satisfied by comments (comment bypass).** A guard asserting a required pattern IS present (e.g., `toContain('.[].inboxRef')`) passes even if the string only appears in a comment and the functional code has been replaced. Filter comments or use a regex that excludes comment lines for positive assertions too.
- **Variable-existence guards ≠ correctness guards.** Asserting that `$redactedOutput` exists in a PS1 file does not prove it is used in the error-emission path. A correctness bypass emits `$cloneOutput` (unredacted) in Write-Error while keeping `$redactedOutput` assigned. For security-sensitive redaction gates, assert `Write-Error[^\n]*\$redactedOutput` to confirm the redacted variable is the one emitted.
...
- **`$ErrorActionPreference = 'Stop'` does NOT apply to native commands in pwsh.** External executables (git, npm, etc.) returning non-zero exit codes do not trigger Stop. The script must check `$LASTEXITCODE` explicitly OR set `$PSNativeCommandUseErrorActionPreference = $true` (PS 7.3+). Mutation tests that rely on native-command failure stopping the script will miss the guard removal.
- **Fixture path must match script-computed path exactly.** If an execution-based test pre-creates a directory to skip a bootstrap step, verify the path the script actually computes (may involve `Split-Path`, `Join-Path`, or `$WorkRoot`-relative logic). A mismatch causes the step to run anyway — which can be harmless but means the guard isn't exercised.
- **EXPECTED_FEATURES gate is an Astro-build gate.** The `EXPECTED_FEATURES` array in `test/docs-build.test.ts` only drives the `all expected doc pages produce HTML in dist/` test, which is behind `requireBuild()`. Adding a feature to EXPECTED_FEATURES is the correct fix for docs-test sync, but the gate is inoperative in environments without Astro installed. Pre-existing infrastructure gap; not a revision regression.
- **Piece-30 revision follow-on (2026-06-02):** APPROVE. FIDO M1 RESOLVED — real pwsh execution test added, passes. PAO M1 RESOLVED — state-backends in EXPECTED_FEATURES. 2 non-blocking nits (NB1 remote-add mutation blind spot; NB2 fixture path mismatch). Test count 196 verified.
...
📌 **Team update (2026-06-03 — Piece 30 Revision-3 Convergence):** Participated in round-3 adversarial review of EECOM's revision (commit 3c6c9edf). Verdict: APPROVE. 212 tests verified (200 ADO-suite); mirror byte-identity PASS. Zero new mandatory findings. 3 new non-blocking refinement items (NB1/NB2/NB3 on gate mutability); none block merge. Convergence achieved; piece-30 Phase B complete.
