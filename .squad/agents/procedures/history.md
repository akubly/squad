...
### Piece-34 kickoff authoring: registry-first topology reconciliation and test-file create/extend decisions (2026-06-06)

Authored the piece-34 client-side publish triggers kickoff. Three reconciliations the kickoff captures that deviate from the spec text: (1) `docsRepoPath` is not a registry field — the docs-repo clone path is `path.dirname(entry.path)`; the spec's config.json-primary language is stale and the kickoff makes registry-first resolution a hard constraint, with config.json as fallback only; (2) two test files the spec lists as "extend" targets (`test/cli/install-hooks.test.ts`, `test/cli/sync-command.test.ts`) do not exist on the piece-33 base branch — the kickoff directs the implementer to CREATE them; three other spec-referenced test files (`assign.test.ts`, `cross-repo-sync.test.ts`, `sync-registry-resolution.test.ts`) do exist — the kickoff correctly retains extend/no-change for those; (3) `--quiet` is already wired by piece 33 — sub-proposal C's `--quiet` task is DONE and must not be re-implemented. Additionally: `packages/squad-cli/src/commands/assign.ts` is the real path (not `src/cli/commands/assign.ts` as the spec's Files table implies); `RunAssignOpts` lacks `developerAlias` and must be extended; sub-proposal B's external Copilot CLI hook API was not found at kickoff time — the kickoff instructs the implementer to re-probe at execution time and record the determination in `decisions/inbox/` before commit.
