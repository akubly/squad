 ...
 * bootstrap utility to depend on @wifi-aware/squad-sdk. This file runs
...
describe('detect-squad-dir zero-dependency guard', () => {
  ...
  it('must not import from @wifi-aware/squad-sdk', () => {
    expect(source).not.toMatch(/from\s+['"]@wifi-aware\/squad-sdk(?:\/[^'"]+)?['"]/);
  });

  it('must not require @wifi-aware/squad-sdk', () => {
    expect(source).not.toMatch(/require\s*\(\s*['"]@wifi-aware\/squad-sdk(?:\/[^'"]+)?['"]\s*\)/);
  });
