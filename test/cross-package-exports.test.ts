 ...
 *
 * Maintenance:
 *   When a new import from @wifi-aware/squad-sdk is added to squad-cli,
...
// ─── Root barrel: @wifi-aware/squad-sdk ─────────────────────────────────

describe('cross-package exports — CLI → SDK', () => {
  describe('@wifi-aware/squad-sdk (root barrel)', () => {
    ...
    it('exports RemoteBridge', async () => {
      const sdk = await import('@wifi-aware/squad-sdk');
      expectExports(sdk, ['RemoteBridge'], '@wifi-aware/squad-sdk');
    });
  });

  // ─── Subpath: /config ──────────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/config', () => {
    it('exports config helpers used by CLI', async () => {
      const mod = await import('@wifi-aware/squad-sdk/config');
      expectExports(mod, [
        'initSquad',
        'MigrationRegistry',
        'writeEconomyMode',
        'readEconomyMode',
      ], '@wifi-aware/squad-sdk/config');
    });
  });

  // ─── Subpath: /config/agent-source ─────────────────────────────────────

  describe('@wifi-aware/squad-sdk/config/agent-source', () => {
    it('exports LocalAgentSource', async () => {
      const mod = await import('@wifi-aware/squad-sdk/config/agent-source');
      expectExports(mod, ['LocalAgentSource'], '@wifi-aware/squad-sdk/config/agent-source');
    });
  });

  // ─── Subpath: /resolution ──────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/resolution', () => {
    it('exports resolution helpers', async () => {
      const mod = await import('@wifi-aware/squad-sdk/resolution');
      expectExports(mod, [
        'resolveSquad',
        'resolveSquadPaths',
        'resolveGlobalSquadPath',
        'resolvePersonalSquadDir',
        'ensurePersonalSquadDir',
      ], '@wifi-aware/squad-sdk/resolution');
    });
  });

  // ─── Subpath: /client ──────────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/client', () => {
    it('exports SquadClient', async () => {
      const mod = await import('@wifi-aware/squad-sdk/client');
      expectExports(mod, ['SquadClient'], '@wifi-aware/squad-sdk/client');
    });
  });

  // ─── Subpath: /adapter/errors ──────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/adapter/errors', () => {
    it('exports RateLimitError', async () => {
      const mod = await import('@wifi-aware/squad-sdk/adapter/errors');
      expectExports(mod, ['RateLimitError'], '@wifi-aware/squad-sdk/adapter/errors');
    });
  });

  // ─── Subpath: /agents/personal ─────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/agents/personal', () => {
    it('exports personal-agent helpers', async () => {
      const mod = await import('@wifi-aware/squad-sdk/agents/personal');
      expectExports(mod, [
        'resolvePersonalAgents',
        'mergeSessionCast',
      ], '@wifi-aware/squad-sdk/agents/personal');
    });
  });

  // ─── Subpath: /casting ─────────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/casting', () => {
    it('exports CastingEngine', async () => {
      const mod = await import('@wifi-aware/squad-sdk/casting');
      expectExports(mod, ['CastingEngine'], '@wifi-aware/squad-sdk/casting');
    });
  });

  // ─── Subpath: /platform ────────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/platform', () => {
    it('exports createPlatformAdapter', async () => {
      const mod = await import('@wifi-aware/squad-sdk/platform');
      expectExports(mod, ['createPlatformAdapter'], '@wifi-aware/squad-sdk/platform');
    });
  });

  // ─── Subpath: /ralph ───────────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/ralph', () => {
    it('exports RalphMonitor', async () => {
      const mod = await import('@wifi-aware/squad-sdk/ralph');
      expectExports(mod, ['RalphMonitor'], '@wifi-aware/squad-sdk/ralph');
    });
  });

  describe('@wifi-aware/squad-sdk/ralph/triage', () => {
    it('exports triage helpers', async () => {
      const mod = await import('@wifi-aware/squad-sdk/ralph/triage');
      expectExports(mod, [
        'parseRoster',
        'parseRoutingRules',
        'parseModuleOwnership',
        'triageIssue',
      ], '@wifi-aware/squad-sdk/ralph/triage');
    });
  });

  describe('@wifi-aware/squad-sdk/ralph/rate-limiting', () => {
    it('exports rate-limiting helpers', async () => {
      const mod = await import('@wifi-aware/squad-sdk/ralph/rate-limiting');
      expectExports(mod, [
        'PredictiveCircuitBreaker',
        'getTrafficLight',
      ], '@wifi-aware/squad-sdk/ralph/rate-limiting');
    });
  });

  // ─── Subpath: /runtime/* ───────────────────────────────────────────────

  describe('@wifi-aware/squad-sdk/runtime/event-bus', () => {
    it('exports EventBus', async () => {
      const mod = await import('@wifi-aware/squad-sdk/runtime/event-bus');
      expectExports(mod, ['EventBus'], '@wifi-aware/squad-sdk/runtime/event-bus');
    });
  });
