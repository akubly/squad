...
**Managed-host freshness (managed entries only):** If the resolved team root is a CLI-managed host — its `registry.json` entry has `managed: true` (a host clone under `~/.squad/hosts/<callsign>/`) — keep it fresh with a deterministic pull at session start. Trigger policy (H2 — sentinel-age threshold): read the age of the `.squad/.last-hydrate-sha` sentinel and run `squad sync --pull` only when it is older than the freshness window (default **4 hours**, overridable via the `SQUAD_MANAGED_FRESH_WINDOW` environment variable in minutes); if the sentinel is missing, pull unconditionally. An explicit user `squad sync --pull` always overrides the threshold. Do NOT pull for non-managed (user working-tree) entries — those are the user's own checkout and are refreshed manually. This freshness check reads and writes only ephemeral state; it makes no durable charter change.
...
prompt: |
  ...
  7. GIT COMMIT (orphan): Stage state files from {SQUAD_DIR} and commit to the `squad-state` orphan branch.
     ...
     d. Push orphan branch: `git push origin squad-state`
        ↳ Pull-before-push (managed hosts): if this team root is a `managed: true` host, first integrate any remote state the freshness sync may have advanced — run `squad sync --pull` (or fetch + fast-forward the namespaced `squad/state/<callsign>` state ref recorded on the managed registry entry, NOT the legacy `squad-state` branch) before pushing so a concurrent hydrate does not cause a non-fast-forward rejection. On non-managed hosts this step is a no-op.
