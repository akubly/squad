...
0. **Clean stale SDK before building.** Before running `npm run build`, remove any
   stale published copy of `@wifi-aware/squad-sdk` that may have been installed
   ...
   ```powershell
   $stale = "packages\squad-cli\node_modules\@wifi-aware\squad-sdk"
   ...
   a no-op. The root `node_modules\@wifi-aware\squad-sdk` workspace symlink remains
